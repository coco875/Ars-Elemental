...
public class HomingPrismLens extends AbstractPrismLens {
    ...
    public void shoot(ServerLevel world, BlockPos pos, EntityProjectileSpell spell, Vec3 angle) {
        ...
        List<Predicate<LivingEntity>> ignore = MethodHomingProjectile.basicIgnores(spell.spellResolver.spellContext.getUnwrappedCaster(world), true, spell.spellResolver.spell);
        newProjectile.setIgnored(ignore);
        newProjectile.setColor(spell.getParticleColor().toWrapper());
