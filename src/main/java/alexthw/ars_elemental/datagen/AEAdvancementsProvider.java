...
import alexthw.ars_elemental.datagen.Advancements;
import alexthw.ars_elemental.datagen.advancement.ANAdvancementBuilder;
import alexthw.ars_elemental.datagen.advancement.ANAdvancements;
...
public class AEAdvancementsProvider extends Advancements {
    ...
    public static class AEAdvancements extends ANAdvancements {
        ...
        public void accept(Consumer<Advancement> con) {
            ...
            builder("mirror_shield").display(ItemsRegistry.ENCHANTERS_SHIELD, FrameType.CHALLENGE, true).addCriterion(new PlayerInteractTrigger.TriggerInstance(ModAdvTriggers.MIRROR.getId(), EntityPredicate.Composite.ANY)).parent(dummy("enchanting_apparatus")).save(con);
            ...
            //path of Air
            builder("levitation").display(ModItems.AIR_CTOME.get(), FrameType.GOAL).addCriterion(new PlayerInteractTrigger.TriggerInstance(ModAdvTriggers.LEVITATE.getId(), EntityPredicate.Composite.ANY)).parent(air).save(con);

            //path of Earth
            builder("spore_blossom").display(ModItems.GROUND_BLOSSOM.get(), FrameType.GOAL).addCriterion(new PlayerInteractTrigger.TriggerInstance(ModAdvTriggers.BLOSSOM.getId(), EntityPredicate.Composite.ANY)).parent(earth).save(con);
