...
public class ANAdvancements implements Consumer<Consumer<Advancement>> {
    ...
    public void accept(Consumer<Advancement> con) {
        this.advCon = con;
        Advancement root = builder(ArsNouveau.MODID).display(ItemsRegistry.WORN_NOTEBOOK, new TranslatableComponent("ars_nouveau.advancement.title.root"),
                new TranslatableComponent("ars_nouveau.advancement.desc.root"),
        ...
        //Advancement poofMob = builder("poof_mob").display(Items.GOLD_NUGGET, FrameType.TASK).addCriterion(new PlayerInteractTrigger.TriggerInstance(ANCriteriaTriggers.POOF_MOB.getId(), EntityPredicate.Composite.ANY)).parent(root).save(con);
        ...
        //builder("wilden_explosion").display(ItemsRegistry.WILDEN_TRIBUTE, FrameType.CHALLENGE, true).addCriterion(new PlayerInteractTrigger.TriggerInstance(ANCriteriaTriggers.CHIMERA_EXPLOSION.getId(), EntityPredicate.Composite.ANY)).parent(tribue).save(con);
        ...
        //builder("familiar").display(ArsNouveauAPI.getInstance().getRitualItemMap().get(new ResourceLocation(ArsNouveau.MODID, RitualLib.BINDING)), FrameType.GOAL).addCriterion(new PlayerInteractTrigger.TriggerInstance(ANCriteriaTriggers.FAMILIAR.getId(), EntityPredicate.Composite.ANY)).parent(rituals).save(con);
        //var jars = saveBasicItem(BlockRegistry.MOB_JAR, rituals);
        //builder("shrunk_starbuncle").display(ItemsRegistry.STARBUNCLE_CHARM, FrameType.CHALLENGE, true).addCriterion(new PlayerInteractTrigger.TriggerInstance(ANCriteriaTriggers.SHRUNK_STARBY.getId(), EntityPredicate.Composite.ANY)).parent(jars).save(con);
        ...
        //builder("prismatic").display(BlockRegistry.SPELL_PRISM, FrameType.CHALLENGE, true).addCriterion(new PlayerInteractTrigger.TriggerInstance(ANCriteriaTriggers.PRISMATIC.getId(), EntityPredicate.Composite.ANY)).parent(prism).save(con);
        ...
        //builder("create_portal").display(BlockRegistry.CREATIVE_SOURCE_JAR, FrameType.CHALLENGE, false).addCriterion(new PlayerInteractTrigger.TriggerInstance(ANCriteriaTriggers.CREATE_PORTAL.getId(), EntityPredicate.Composite.ANY)).parent(warpScroll).save(con);
