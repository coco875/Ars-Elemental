...
public class AEPatchouliProvider extends PatchouliProvider {
    ...
    public void run(HashCache cache) throws IOException {
        ...
        addPage(new PatchouliBuilder(ENCHANTMENTS, ItemsRegistry.NOVICE_SPELLBOOK).withTextPage("ars_elemental.page.book_protection").withName("tooltip.ars_nouveau.blessed").withPage(new ApparatusPage(prefix("invincible_book").toString())), getPath(ENCHANTMENTS, "invincible_book"));
    ...
    public void addFamiliarPage(AbstractFamiliarHolder familiarHolder) {
        PatchouliBuilder builder = new PatchouliBuilder(FAMILIARS, "entity.ars_elemental." + familiarHolder.getId())
                .withIcon("ars_elemental:" + familiarHolder.getId())
                .withTextPage("ars_elemental.familiar_desc." + familiarHolder.getId())
                .withPage(new EntityPage(familiarHolder.getId().toString()));
        this.pages.add(new PatchouliPage(builder, getPath(FAMILIARS, familiarHolder.getId())));
    }

    public void addRitualPage(AbstractRitual ritual) {
        PatchouliBuilder builder = new PatchouliBuilder(RITUALS, "item.ars_elemental." + ritual.getID())
                .withIcon(ritual.getID().toString())
                .withTextPage(ritual.getDescription())
                .withPage(new CraftingPage("ars_elemental:tablet_" + ritual.getID()));
    ...
    public void addGlyphPage(AbstractSpellPart spellPart) {
        ...
        PatchouliBuilder builder = new PatchouliBuilder(category, spellPart.getName())
                .withName("ars_elemental.glyph_name." + spellPart.getId())
                .withIcon(spellPart.getId().toString())
                .withSortNum(spellPart instanceof AbstractCastMethod ? 1 : spellPart instanceof AbstractEffect ? 2 : 3)
                .withPage(new TextPage("ars_elemental.glyph_desc." + spellPart.getId()))
                .withPage(new GlyphPressPage(spellPart));
        this.pages.add(new PatchouliPage(builder, getPath(category, spellPart.getId())));
    }
